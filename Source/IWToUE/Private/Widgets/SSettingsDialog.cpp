...
void SSettingsDialog::Construct(const FArguments& InArgs, UWraithSettings* InSettingsModel)
{
	check(InSettingsModel != nullptr);
	SettingsModel = InSettingsModel;

	GeneralViewModel = MakeShared<FGeneralSettingsViewModel>(SettingsModel);
	ModelViewModel = MakeShared<FModelSettingsViewModel>(SettingsModel);
	AnimationViewModel = MakeShared<FAnimationSettingsViewModel>(SettingsModel);
	MaterialViewModel = MakeShared<FMaterialSettingsViewModel>(SettingsModel);
	TextureViewModel = MakeShared<FTextureSettingsViewModel>(SettingsModel);
	AudioViewModel = MakeShared<FAudioSettingsViewModel>(SettingsModel);
	MapViewModel = MakeShared<FMapSettingsViewModel>(SettingsModel);
	// 创建标签管理器（不关联特定DockTab）
...
void SSettingsDialog::RegisterTabSpawners()
{
	if (!TabManager.IsValid()) return;
	auto CreateTabContent = [](TSharedRef<SWidget> InnerContent)
	{
		return SNew(SBorder)
	...
	TabManager->RegisterTabSpawner(WraithXSettings::TabIds::Map,
								   FOnSpawnTab::CreateRaw(this, &SSettingsDialog::SpawnMapSettingsTab))
...
{
	check(GeneralViewModel.IsValid());
	return SNew(SDockTab)
		.Label(LOC_SETTINGS("GeneralTabTitle", "General"))
		.TabRole(ETabRole::PanelTab)
		[
			SNew(SGeneralSettingsPage, GeneralViewModel.ToSharedRef())
		];
}

TSharedRef<SDockTab> SSettingsDialog::SpawnModelSettingsTab(const FSpawnTabArgs& Args)
{
	check(ModelViewModel.IsValid());
	return SNew(SDockTab)
		.Label(LOC_SETTINGS("ModelTabTitle", "Model"))
		.TabRole(ETabRole::PanelTab)
		[
			SNew(SModelSettingsPage, ModelViewModel.ToSharedRef())
		];
}

TSharedRef<SDockTab> SSettingsDialog::SpawnAnimationSettingsTab(const FSpawnTabArgs& Args)
{
	check(AnimationViewModel.IsValid());
	return SNew(SDockTab)
		.Label(LOC_SETTINGS("AnimTabTitle", "Animation"))
		.TabRole(ETabRole::PanelTab)
		[
			SNew(SAnimationSettingsPage, AnimationViewModel.ToSharedRef())
		];
}

TSharedRef<SDockTab> SSettingsDialog::SpawnMaterialSettingsTab(const FSpawnTabArgs& Args)
{
	check(MaterialViewModel.IsValid());
	return SNew(SDockTab)
		.Label(LOC_SETTINGS("MaterialTabTitle", "Material"))
		.TabRole(ETabRole::PanelTab)
		[
			SNew(SMaterialSettingsPage, MaterialViewModel.ToSharedRef())
		];
}

TSharedRef<SDockTab> SSettingsDialog::SpawnTextureSettingsTab(const FSpawnTabArgs& Args)
{
	check(TextureViewModel.IsValid());
	return SNew(SDockTab)
		.Label(LOC_SETTINGS("TextureTabTitle", "Texture"))
		.TabRole(ETabRole::PanelTab)
		[
			SNew(STextureSettingsPage, TextureViewModel.ToSharedRef())
		];
}

TSharedRef<SDockTab> SSettingsDialog::SpawnAudioSettingsTab(const FSpawnTabArgs& Args)
{
	check(AudioViewModel.IsValid());
	return SNew(SDockTab)
		.Label(LOC_SETTINGS("AudioTabTitle", "Audio"))
		.TabRole(ETabRole::PanelTab)
		[
			SNew(SAudioSettingsPage, AudioViewModel.ToSharedRef())
		];
}

TSharedRef<SDockTab> SSettingsDialog::SpawnMapSettingsTab(const FSpawnTabArgs& Args)
{
	check(MapViewModel.IsValid());
	return SNew(SDockTab)
		.Label(LOC_SETTINGS("MapTabTitle", "Map"))
		.TabRole(ETabRole::PanelTab)
		[
			SNew(SMapSettingsPage, MapViewModel.ToSharedRef())
		];
}
